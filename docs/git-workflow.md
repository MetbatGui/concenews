...
Ready PR을 연 뒤 Codex는 별도 컨텍스트의 읽기 전용 리뷰를 자동으로 수행하고, `# Independent Review` 댓글로 결과를 남긴다. Draft PR은 구현 진행 상황 공유에만 사용하며 독립 리뷰를 시작하지 않는다.
...
4. 수정 뒤 `just check-branch-green`·커밋·push를 수행하고, `# Independent Review 조치` 댓글로 변경·검증 결과를 남긴다.
